...
export const refreshToken = (collections: Promise<beforeAll>): void =>
    ...
    it("test that request failed and a new token is requested, and the new token is saved", async (): Promise<void> => {
      const rawResponse1: any = new Promise((resolve) => {
        page.on("response", async (response) => {
          if (
            (await response.url()) === "http://192.168.1.111:4000/user/validator" &&
      ...
      // delete from the DB
      const oldAccessToken = (await AccessTokens.find({}).toArray())[0]
        .accessToken;
      ...
      const isAccessTokenExist = Boolean(
        rawCookies.find((cookie) => cookie.name === "accessToken")
      );
      // test that we succesfully saved a new token in the browser
      expect(isAccessTokenExist).toBe(true);

      const newAcessToken = (await AccessTokens.find({}).toArray())[0]
        .accessToken;
